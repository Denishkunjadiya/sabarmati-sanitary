...
const Balcony = () => {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:5000/api/balcony', formData)
            .then((response) => {
    ...
    const loadData = async () => {
        const response = await axios.get("http://localhost:5000/api/display_balcony");
    ...
    const deletebalcony = (id) => {
        if (window.confirm("are You Syour You Want To Delete That ? ")) {
          axios.delete(`http://localhost:5000/api/delete_balcony/${id}`);
        }
      }
                        ...
                        {data.map((item, index) => {
                            return (
                                <tr key={item.id}>
                                    ...
                                    <td className='action'>
                                        <i class="fa-regular fa-trash-can" onClick={() => deletebalcony(item.id)}></i>
                                    </td>
                                </tr>
                            )
                        })}
