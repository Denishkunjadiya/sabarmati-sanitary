...
const Porcelain = () => {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:5000/api/porcelain', formData)
            .then((response) => {
    ...
    const deleteporcelain = (id) => {
        if (window.confirm("are You Syour You Want To Delete That  ? ")) {
          axios.delete(`http://localhost:5000/api/delete_porcelain/${id}`);
        }
      }
                        ...
                        {data.map((item, index) => {
                            return (
                                <tr key={item.id}>
                                    ...
                                    <td className='action'>
                                        <i class="fa-regular fa-trash-can" onClick={() => deleteporcelain(item.id)}></i>
                                    </td>
                                </tr>
                            )
                        })}
