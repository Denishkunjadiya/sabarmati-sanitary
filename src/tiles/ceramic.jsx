...
const Ceramic = () => {
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:5000/api/ceramic', formData)
            .then((response) => {
    ...
    const loadData = async () => {
        const response = await axios.get("http://localhost:5000/api/display_ceramic");
    ...
    const deleteceramic = (id) => {
        if (window.confirm("are You Syour You Want To Delete That  ? ")) {
          axios.delete(`http://localhost:5000/api/delete_ceramic/${id}`);
        }
      }
    return (
        <div>
                            ...
                            return (
                                <tr key={item.id}>
                                    ...
                                    <td className='action'>
                                        ...
                                        <i class="fa-regular fa-trash-can" onClick={() => deleteceramic(item.id)}></i>
                                    </td>
                                </tr>
                            )
                        })}
